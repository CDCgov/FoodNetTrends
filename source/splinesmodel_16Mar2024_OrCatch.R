...
# chmod u+x splinesmodel_script
# nohup ./splinesmodel_script OR bash splinesmodel_script OR ./splinesmodel_script
# tail -f nohup.out
...
run<-"12Apr2024_MMWRR1_OrCatchment"
...
mmwrdata<-haven::read_sas("/scicomp/groups-pure/OID/NCEZID/DFWED/EDEB/foodnet/trends/data/mmwr9623.sas7bdat")%>% # input surveillance
            filter(SiteID!="COEX")

# Do we want the Expanded Colorado Catchment

# Run the fnd function to Aggregate Data by State and Year
fnd<-function(travel, cidt, data){
fn<-data %>%
...
trendy<-function(data){ 
  ...
  target<-unique(data$pathogen)
  brm(count~ s(yearn, by=state) + # interaction(state, culture)
                 state + offset(log(population)), data=data, family = "negbinomial",
      save_all_pars=TRUE, chains=6, iter=5001, seed=47, 
      cores=6,
      control = list(adapt_delta = 0.999, max_treedepth = 19)) ->  proposed
  saveRDS(proposed, paste("~/2020.FoodNetSplines/mmwr_output/",unique(data$pathogen), travel, culture, run,".Rds", sep=""))
...
from_linpred_catchIR<-function(catchments){
  ...
  # save estimates
  # write.table(catchir.linpred%>%select(-c(.value_LL, .value_UL)), file = paste("/scicomp/groups-pure/OID/NCEZID/DFWED/EDEB/foodnet/trends/mmwroutput/DistrofEstIR", run,".csv", sep=""), append = TRUE, quote = TRUE, sep =",", qmethod = "double", col.names = TRUE, row.names = TRUE)
...
#############
  ...
  threeyear$endyear<-2018
  write.table(threeyear, paste("/scicomp/groups-pure/OID/NCEZID/DFWED/EDEB/foodnet/trends/mmwroutput/EstRR", run,".csv", sep=""), append = TRUE, quote = TRUE, sep =",", qmethod = "double", col.names = TRUE, row.names = TRUE)
  ...
  recent$endyear<-2022
  write.table(recent, paste("/scicomp/groups-pure/OID/NCEZID/DFWED/EDEB/foodnet/trends/mmwroutput/EstRR", run,".csv", sep=""), append = TRUE, quote = TRUE, sep =",", qmethod = "double", col.names = FALSE, row.names = TRUE)
  ...
  finalcatchment$endyear<-2006
  write.table(finalcatchment, paste("/scicomp/groups-pure/OID/NCEZID/DFWED/EDEB/foodnet/trends/mmwroutput/EstRR", run,".csv", sep=""), append = TRUE, quote = TRUE, sep =",", qmethod = "double", col.names = FALSE, row.names = TRUE)
  ...
  baseline$endyear<-2008
  write.table(baseline, paste("/scicomp/groups-pure/OID/NCEZID/DFWED/EDEB/foodnet/trends/mmwroutput/EstRR", run,".csv", sep=""), append = TRUE, quote = TRUE, sep =",", qmethod = "double", col.names = FALSE, row.names = TRUE)
}
...
bact<-fnd(travel, cidt, mmwrdata) # aggregates annually from 1996 to Nth year for 9 diseases in 10 states = 10*9*(N-1996) : aggregates annually from 1996 to Nth year for 9 diseases in Y counties = 10*9*(N-1996)
bact$yearn<-as.numeric(as.character(bact$year))
bact$year<-as.factor(bact$year)
# bact<-bact%>%filter(pathogen=="STEC O157" | pathogen=="SALMONELLA" | pathogen=="STEC NONO157")
# bact<-bact%>%filter(pathogen=="SALMONELLA" | pathogen=="LISTERIA")
# bact<-bact%>%filter(pathogen=="LISTERIA")

bact2<-split(bact, bact$pathogen)
# write_rds(bact2, paste("~/2020.FoodNetSplines/MMWRDataCleaned_AllCases",run,".Rds", sep=""))
...
bact2<-split(bact, bact$pathogen)
write_rds(bact2, paste("~/2020.FoodNetSplines/MMWRDataCleaned_UnkNoTravelCxCIDT",run,".Rds", sep=""))
...
#############
# Pathogens with Travel =="No" and CIDT
#############
...
bact2<-split(bact, bact$pathogen)
write_rds(bact2, paste("~/2020.FoodNetSplines/MMWRDataCleaned_NoTravelCxCIDT",run,".Rds", sep=""))
...
#############
# Pathogens with Travel but not CIDT
#############

# save.image(file='~/2020.FoodNetSplines/mmwr_output/test/testingparallel.RData')
# load(file='~/2020.FoodNetSplines/mmwr_output/test/testingparallel.RData')
start.time <- Sys.time()
travel<-c("NO", "UNKNOWN", "YES")
cidt<-c("CX+", "PARASITIC")

bact<-fnd(travel, cidt, mmwrdata)
# table(bact$year, bact$pathogen)
# bact2<-bact%>% group_by(year, pathogen) %>% dplyr::summarize(count =sum(count, na.rm=TRUE), population =sum(population, na.rm=TRUE))
bact$yearn<-as.numeric(as.character(bact$year))
bact$year<-as.factor(bact$year)

bact2<-split(bact, bact$pathogen)
write_rds(bact2, paste("~/2020.FoodNetSplines/MMWRDataCleaned_Cx",run,".Rds", sep=""))
